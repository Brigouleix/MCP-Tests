...
def list_emails(max_results: Any = 5):
    """List ONLY received emails from the INBOX."""
    try:
    ...
    try:
        ...
        # -> label:INBOX : uniquement la boite de réception
        # -> -from:me : EXCLUT tous les messages dont l'expéditeur est "moi"
        ...
        for msg in messages:
            ...
            # Double sécurité : on vérifie aussi ici si le message ne vient pas de "me"
...
def _ask_llama(content, prompt_type="summary"):
    """Internal function to prompt Llama 3.2 in English for better accuracy."""
    # Nettoyage des balises techniques
    clean = content.replace("BODY_START", "").replace("BODY_END", "").strip()
    
    if prompt_type == "summary":
        # Prompt pour un résumé structuré et court
        p = (
            "Summarize the following email concisely in 3 bullet points maximum. "
            "Focus on the main topic and any required action. "
            f"Email content:\n\n{clean}"
        )
    else:
        # Prompt pour une réponse pro sans bavardage
        p = (
            "Write a professional and warm reply to this email. "
            "Output ONLY the message body. Do not add any introductory remarks like 'Here is the draft'. "
            f"Original email:\n\n{clean}"
        )
    ...
    try:
        response = ollama.chat(model='llama3.2', messages=[
            {
                'role': 'system', 
                'content': 'You are a highly efficient Gmail assistant. You write directly and professionally.'
            },
            {'role': 'user', 'content': p}
        ])
        ...
        return "AI analysis error. Please check if Ollama is running."

@mcp.tool()
def smart_analyze_email(message_id: str):
    """Récupère et analyse l'email."""
